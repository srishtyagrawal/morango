...
class Test(unittest.TestCase) :
	...
	def test_deserialize(self) :
		...
		self.assertEqual(node.appData[1], ("record1", "record1 data", 0, Node.ALL, Node.ALL))

		node.appData.append(("record3", "record3 old data", 0, Node.ALL, Node.ALL))
		#Add same record to node's store
		node.store["record3"] = StoreRecord("record3", "record3 new data", "D", 1, {"D":1, "A":1}, Node.ALL, Node.ALL)

		node.deserialize()
		self.assertEqual(len(node.appData), 3)
		self.assertEqual(len(node.store), 2)
		# Check if new data has succesfully been replaced by old data
		self.assertEqual(node.appData[2], ("record3", "record3 new data", 0, Node.ALL, Node.ALL))

		node.addAppData("record4", "record4 data generate by A", Node.ALL, Node.ALL)		
		node.store["record4"] = StoreRecord("record4", "record4 data generated by D", "D", 2, {"D":2}, Node.ALL, Node.ALL)
